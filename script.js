...
function parserUsers(data) {
    for (const key in data) {
        const {name} = data[key]   /* vot eto deskturizatsiya */
        const {username} = data[key]
        const {email} = data[key]
        const {address = street} = data[key]
        const {phone} = data [key]
        const {website} = data [key]
        const {company = name} = data [key]
        console.info(` Ismi:   ${name}\n Niki: ${username}\n Email: ${email}\n Manzil: ${address.street}\n komplekt: ${address.suite}\n City: ${address.city}\n zip-cod: ${address.zipcode}\n geopozitsiya: ${address.geo.lat}   lng: ${address.geo.lng}\n nomer: ${phone}\n web-site ${ website}\n companiyani-ismi:${company.name}\n catchPhrase:${company.catchPhrase}\n BS: ${company.bs} \n`)}
}